...
ggmap(map) + geom_point(data = airportflightcount, aes(x = longitude, y = latitude, size = airportflightcount, colour = airline_id)) + guides(fill='legend', size=F) + scale_size(range = c(0, 6)) + scale_fill_gradient(low = "#ff0000", high = "#04ff00")
...
airline_type_vs_price <- ggplot(data = kmeansdata, aes(x=airline_type)) + geom_jitter(aes(y = avgPrice,  colour = airline_type),  size=1, alpha=0.01) + labs(title="Average Ticket Price By Airline Type",  x= "Airline Type", y="Average Price", fill="Airline Type") + theme(legend.position="none")
